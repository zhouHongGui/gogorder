...
CREATE TABLE `refund_ledger` (
  ...
  `idempotent_key` VARCHAR(64) NOT NULL COMMENT '幂等键：{orderId}:refund:{seq}',
  `status` TINYINT DEFAULT 1 COMMENT '1成功',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_idempotent` (`idempotent_key`),
...
CREATE TABLE `stock_ledger` (
  ...
  `idempotent_key` VARCHAR(64) NOT NULL COMMENT '幂等键：{orderId}:stock',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_idempotent` (`idempotent_key`),
...
CREATE TABLE `biz_order` (
  ...
  `pickup_token` VARCHAR(8) NOT NULL COMMENT '内部核销令牌（全局唯一UUID短码）',
  `pickup_display` VARCHAR(5) NOT NULL COMMENT '门店当日展示号（5位流水）',
  `user_id` BIGINT NOT NULL,
  `shop_id` BIGINT NOT NULL,
  `order_status` TINYINT NOT NULL DEFAULT 0 COMMENT '0待支付 1已支付 2制作中 3待取餐 4已完成 5已取消',
  ...
  UNIQUE KEY `uk_pickup_display` (`shop_id`, `pickup_display`, `create_time`),
  KEY `idx_user_id` (`user_id`),
...
```
pickup:seq:{shopId}:{yyyyMMdd} → INCR，每日自动过期
```
...
| biz_order | `uk_pickup_display (shop,code,time)` | 展示号防碰撞 |
